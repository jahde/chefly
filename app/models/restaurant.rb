...
class Restaurant < ActiveRecord::Base
  ...
  def self.data_dump
    ...
    @neighborhoods.each do |neighborhood|
      ...
      neighborhood.zipcodes.each do |zip|
        response = HTTParty.get("http://api.locu.com/v1_0/venue/search/?api_key=#{ENV['DACE_LOCU']}&postal_code=#{zip}")
  ...
  def get_dishes(menu_id, this_restaurant)
     menus = HTTParty.get("http://api.locu.com/v1_0/venue/#{menu_id}/?api_key=#{ENV['DACE_LOCU']}")
        dish_name = ""
        description = ""
